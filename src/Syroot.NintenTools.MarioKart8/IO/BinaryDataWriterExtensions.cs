...
namespace Syroot.NintenTools.MarioKart8.IO
{
    ...
    internal static class BinaryDataWriterExtensions
    {
        ...
        /// <summary>
        /// Writes <see cref="Triangle"/> instances into the current stream.
        /// </summary>
        /// <param name="self">The extended <see cref="BinaryDataWriter"/>.</param>
        /// <param name="values">The <see cref="Triangle"/> instances.</param>
        internal static void Write(this BinaryDataWriter self, Triangle[] values)
        {
            foreach (Triangle value in values)
            {
                self.Write(value.Length);
                self.Write(value.PositionIndex);
                self.Write(value.DirectionIndex);
                self.Write(value.NormalIndex1);
                self.Write(value.NormalIndex2);
                self.Write(value.NormalIndex3);
